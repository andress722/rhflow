...
export default function PresencePage() {
  ...
  useEffect(() => {
    ...
    // Initialize date with local today (civil date)
            ...
            <table className="w-full text-left text-sm text-slate-400 border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/50 text-slate-500 font-semibold text-xs uppercase tracking-wider">
                  ...
                  <th className="px-6 py-3.5">Resposta Recebida</th>
                  <th className="px-6 py-3.5 text-right">Ocorrência</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {filteredCheckins.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {item.responseText ? (
                        <div className="flex items-center gap-2 max-w-xs text-xs text-slate-350 bg-slate-950/40 border border-slate-850/50 py-1 px-2.5 rounded-lg italic">
                    ...
                    <td className="px-6 py-4 text-right">
                      {item.occurrence ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-750 text-indigo-400 hover:text-indigo-350 text-xs font-bold border border-slate-700 transition-colors"
                        >
                          Ver Ocorrência
