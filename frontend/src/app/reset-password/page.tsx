...
import { Lock, ShieldAlert, CheckCircle2, ShieldCheck, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    if (queryToken) {
      ...
      setError('Token de recuperação ausente. Verifique o link recebido por e-mail.');
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFormValid) {
      setError('Por favor, atenda a todos os requisitos de segurança e garanta que o token está presente.');
      return;
    }
    if (isLoading) return;
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.error?.message || 'Erro ao redefinir a senha. O token pode ser inválido ou ter expirado.');
    ...
    <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-8 relative z-10 shadow-2xl">
      {success ? (
        <div className="flex flex-col items-center text-center py-8 space-y-4">
          ...
          <h2 className="text-2xl font-bold text-white">Nova Senha Salva!</h2>
        ...
        <>
          ...
          <div className="mb-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Crie uma nova senha forte para restabelecer o acesso à sua conta.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="p-4 bg-slate-950 rounded-lg border border-slate-800 space-y-2">
              <span className="text-xs font-semibold text-slate-400 block mb-1">Requisitos da Nova Senha:</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                ...
                <div className="flex items-center gap-2 col-span-2">
                  ...
                  <span className={rules.symbol ? 'text-slate-200' : 'text-slate-500'}>Ao menos um símbolo (@, $, !, %, etc.)</span>
                </div>
              </div>
            ...
            <div className="text-center">
              <Link
                href="/login"
                className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-200 text-xs font-medium transition-colors"
              >
